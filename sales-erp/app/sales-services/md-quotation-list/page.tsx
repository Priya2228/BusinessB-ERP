...
export default function MdQuotationListPage() {
  ...
  const openReviewModal = (row) => {
    ...
    setReviewRow(row);
    setReviewForm({
      status: record.md.status === "declined" ? "declined" : "approved",
      comment: record.md.comment || "",
    });
  };
  ...
  const handleReviewSubmit = async () => {
    ...
    try {
      const response = await fetch(buildApiUrl(`/api/quotations/${reviewRow.id}/approval/`), {
        method: "POST",
        headers: {
    ...
    <AppPageShell contentClassName="mx-auto w-full max-w-[1240px] px-3 py-2">
      <div className="mt-3 mx-auto max-w-[1180px] rounded-[20px] border border-sky-100 bg-[#fbfdff] p-4 shadow-[0_8px_28px_rgba(15,23,42,0.05)]">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h1 className="text-[16px] font-bold text-slate-900">MD Quotation List</h1>
          </div>
        ...
        <div className="mt-4 overflow-hidden rounded-[14px] border border-slate-200">
          {loading ? (
            ...
            <div className="px-4 py-12 text-center text-[13px] text-slate-500">No quotations sent to MD.</div>
          ) : (
                    ...
                    return (
                      <tr key={row.id} className="align-top">
                        <td className="border-b border-slate-100 px-4 py-3">
                          <p className="font-semibold text-slate-900">{row.company_name || row.customer_name || "-"}</p>
                        </td>
                        <td className="border-b border-slate-100 px-4 py-3">
                          <p>{row.attention_name || row.customer_name || "-"}</p>
                          <p className="mt-1">{row.phone_no || "-"}</p>
                          <p className="mt-1">{row.email || "-"}</p>
                        </td>
                        <td className="border-b border-slate-100 px-4 py-3">
                          <p>Quote: {row.quotation_code || "-"}</p>
                          <p className="mt-1">RFQ: {row.rfq_no || "-"}</p>
                          <p className="mt-1">Date: {row.quotation_date || "-"}</p>
                        </td>
                            ...
                            {isAdminRole(authRole) ? (
                              <>
                                <button
                                  ...
                                  title={isMdApproved ? "Disabled after MD approval" : "Update quotation"}
                                >
                                  <Pencil size={14} />
                                </button>
                                <button
                                  ...
                                  title={isMdApproved ? "Disabled after MD approval" : "Delete quotation"}
                                >
                                  <Trash2 size={14} />
                                </button>
                              </>
