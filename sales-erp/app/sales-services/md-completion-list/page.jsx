...
import { getApprovalRecord, getStageBadgeClass, getStageLabel } from "../approvalWorkflow";
...
const isCompletionRequest = (service) =>
  (service?.rfq_category || "").toLowerCase() === "quote of completion";

export default function MdCompletionListPage() {
  ...
  const fetchRows = useCallback(async () => {
    try {
      ...
      setError("");
    try {
      await fetch(buildApiUrl("/api/sales-services/ensure-completion-estimations/"), {
        method: "POST",
        headers: getAuthHeaders(),
      });
    } catch {
      // best effort; list will still work if we can't ensure
    }
    ...
    const [serviceResponse, estimationResponse] = await Promise.all([
      ...
      if (!serviceResponse.ok) {
        setError("Failed to fetch completion requests.");
        return;
      }

      const serviceData = await serviceResponse.json();
      const estimationData = estimationResponse.ok ? await estimationResponse.json() : [];
      const estimationMap = (estimationData || []).reduce((acc, estimation) => {
      ...
      const completionRows = (serviceData || [])
        .filter(isCompletionRequest)
        .map((service) => {
          const estimation = estimationMap[service.rfq_no];
          if (!estimation || !estimation.approval_workflow) return null;
          return { service, estimation };
        })
        .filter(Boolean);

      setRows(completionRows);
    } catch {
      setError("Network error while fetching completion requests.");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      setRows((prev) => prev.filter((row) => row.estimation.id !== id));
    } catch {
  ...
  const openReviewModal = (row) => {
    const approvalRecord = getApprovalRecord(row.estimation.approval_workflow);
    setReviewRow(row);
    setReviewForm({
      status: approvalRecord.md.status === "declined" ? "declined" : "approved",
      comment: approvalRecord.md.comment || "",
    });
  };
  ...
  const handleReviewSubmit = async () => {
    ...
    try {
      const response = await fetch(
        buildApiUrl(`/api/cost-estimations/${reviewRow.estimation.id}/approval/`),
        {
          method: "POST",
          headers: {
      ...
      if (!response.ok) {
        ...
        window.alert(errorData.detail || "Failed to save MD approval.");
        return;
      }
      ...
      setRows((prev) =>
        prev.map((candidate) =>
          candidate.estimation.id === reviewRow.estimation.id
            ? { ...candidate, estimation: { ...candidate.estimation, approval_workflow: approval } }
            : candidate
        )
      );
      setReviewRow(null);
    } catch {
      window.alert("Network error while saving MD approval.");
    }
  };
  ...
  const renderStatus = (approvalWorkflow) => {
    ...
    return (
      <div className="flex flex-col items-start gap-1.5">
        <span className={`${statusButtonClassName} ${getStageBadgeClass(record.md.status)}`}>
          {getStageLabel(record.md.status, "WAITING FOR MD APPROVAL")}
        </span>
        <p className="text-[10px] text-slate-500">
          Head Status: {getStageLabel(record.head.status, "WAITING FOR HEAD APPROVAL")}
        </p>
        {record.md.comment ? (
          <p className="text-[10px] text-slate-500">MD: {record.md.comment}</p>
        ) : null}
      </div>
    );
  };

  const visibleRows = rows.filter((row) => getApprovalRecord(row.estimation.approval_workflow).sentToHead);
    ...
    <AppPageShell contentClassName="mx-auto w-full max-w-[1240px] px-3 py-2">
      <div className="mt-3 mx-auto max-w-[1180px] rounded-[20px] border border-sky-100 bg-[#fbfdff] p-4 shadow-[0_8px_28px_rgba(15,23,42,0.05)]">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h1 className="text-[16px] font-bold text-slate-900">MD Completion List</h1>
          </div>
        ...
        <div className="mt-4 overflow-hidden rounded-[14px] border border-slate-200">
          {loading ? (
            ...
            <div className="px-4 py-12 text-center text-[13px] text-slate-500">
              No completion cost estimations sent to MD.
            </div>
          ) : (
              ...
              <table className="min-w-[1240px] w-full border-collapse text-left text-[11px] text-slate-700">
                ...
                <tbody className="bg-white">
                  {visibleRows.map((row) => {
                    ...
                    const isMdApproved = approvalRecord.md.status === "approved";
                    return (
                      <tr key={row.estimation.id} className="align-top">
                        <td className="border-b border-slate-100 px-4 py-3">
                          <p className="font-semibold text-slate-900">{row.service.company_name || "-"}</p>
                        </td>
                        <td className="border-b border-slate-100 px-4 py-3">
                          <p>{row.service.client_name || "-"}</p>
                          <p className="mt-1">{row.service.phone_no || "-"}</p>
                          <p className="mt-1">{row.service.email || "-"}</p>
                        </td>
                        <td className="border-b border-slate-100 px-4 py-3">
                          <p>Est: {row.estimation.estimation_no || "-"}</p>
                          <p className="mt-1">RFQ: {row.service.rfq_no || "-"}</p>
                          <p className="mt-1">Date: {row.service.registered_date || "-"}</p>
                        </td>
                        <td className="border-b border-slate-100 px-4 py-3">
                          {renderStatus(row.estimation.approval_workflow)}
                        </td>
                        <td className="border-b border-slate-100 px-4 py-3">
                          <div className="flex items-center gap-2">
                            {canApproveCostEstimation(authRole) ? (
                              <button
                                ...
                                title={isHeadApproved ? "MD review" : "Head approval required first"}
                              >
                            ...
                            {isAdminRole(authRole) ? (
                              <>
                                <button
                                  type="button"
                                  onClick={() =>
                                    router.push(`/sales-services/cost-sheet?editId=${row.estimation.id}`)
                                  }
                                  disabled={isMdApproved}
                                  className={`flex h-7 w-7 items-center justify-center rounded-md border transition ${
                                    isMdApproved
                                      ? "cursor-not-allowed border-slate-200 bg-slate-100 text-slate-400"
                                      : "border-amber-200 bg-amber-50 text-amber-600"
                                  }`}
                                  title={isMdApproved ? "Disabled after MD approval" : "Update cost estimation"}
                                >
                                  <Pencil size={14} />
                                </button>
                                <button
                                  type="button"
                                  onClick={() => handleDelete(row.estimation.id)}
                                  disabled={isMdApproved}
                                  className={`flex h-7 w-7 items-center justify-center rounded-md border transition ${
                                    isMdApproved
                                      ? "cursor-not-allowed border-slate-200 bg-slate-100 text-slate-400"
                                  ...
                                  title={isMdApproved ? "Disabled after MD approval" : "Delete cost estimation"}
                                >
                                  <Trash2 size={14} />
                                </button>
                              </>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/25 px-4">
          <div className="w-full max-w-[420px] rounded-[20px] border border-slate-200 bg-white p-5 shadow-[0_24px_50px_rgba(15,23,42,0.18)]">
            <div className="flex items-center justify-between">
              <h2 className="text-[16px] font-bold text-slate-900">MD Approval</h2>
              <button
                type="button"
